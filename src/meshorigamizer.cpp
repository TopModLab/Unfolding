...
bool MeshOrigamizer::origamiMesh( HDS_Mesh * mesh )
{
	...
	int markIndex = 0;
	auto rotateFace = [&rotationMap, &flipMap, &rotateVector2D, &markQueue, &vertMarked, &markIndex] (he_t* he, QVector2D vec) -> void {
		if (he->f->isMarked)
			return;
		he->f->isMarked = true;
		he->f->markNumber = ++markIndex;
		he_t* curHe = he;
	...
	auto clockWiseRotate = [](QVector2D before, QVector2D after) -> double {
		...
		double angle = acos(clamp<double>(cosVal, -1.0, 1.0));
		double sinVal = (after.x() * before.y() - after.y() * before.x()) / pow(before.length() ,2);
		if (sinVal < -0.000001) {
			angle = PI2 - angle;
		}
		return angle;
	...
	vert_t* start_vert = hes_new[0]->v;
	he_t* start_he = hes_new[0];

	for (auto vert : verts_new) {
		start_he = vert->he;
		if (flipMap.find(start_he) == flipMap.end())
			start_he = start_he->flip;
		he_t* cur_he = start_he;
		bool found = false;
		while (!cur_he->isCutEdge) {
			cur_he = flipMap[cur_he]->next;
			if (cur_he == start_he) {
				found = true;
				break;
			}
	...
	markQueue.push(start_vert->refid);
	rotateFace(start_he, QVector2D(1, 0));
	...
	while (!markQueue.empty()) {
		...
		while (!curHe->f->isMarked) {
			...
			//cout << curHe->refid << endl;
		}
		he_t* startHe = curHe;
		cout << "-------------- Vert: " << (vertId >> 2) << " -------------" << endl;
		do {
			...
			while (!curFlip->f->isMarked) {
				...
				QVector3D v1 = curFlip->v->pos - curFlip->flip->v->pos;
				QVector3D v2 = nextHe->flip->v->pos - nextHe->v->pos;
			...
			cout << "meshAngleSum: " << meshAngleSum << " -- totalAngleSum: " << totalAngleSum << "  --  # of angle to be distributed: " << flipsToRotate.size() + 1 << endl;
			...
			for (auto& he : flipsToRotate) {
				...
				cout << "rotatedVec: " << -rotatedVec.x() << " -- " << -rotatedVec.y() << endl;
	...
	for (auto& heMap : refidMap) {
		...
		if (!he->isCutEdge) {
			...
			//double angle = clockWiseRotate(rotationMap[he->flip], -rotationMap[flipMap[he->flip]]);
			...
			if (angle <= Pi) {
				...
				angle /= 2;
				angle += Pi / 2;
				addBridger(he->bridgeTwin, he, angle, tucked_length, tucked_smooth);
			}
