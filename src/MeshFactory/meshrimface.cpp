...
void MeshRimFace::projectFaceCenter(
	vert_t* v, he_t* he, QVector3D &vn_pos, QVector3D &vp_pos)
{
...
void MeshRimFace::projectEdgeVertices(
	vert_t* v, he_t* he, QVector3D &up_pos, QVector3D &down_pos)
{
#ifdef USE_LEGACY_FACTORY
	QVector3D n = he->computeNormal();
...
void MeshRimFace::computePlaneCornerOnEdge(
	vert_t* v, he_t* he, vector<QVector3D> &vpos)
{
#ifdef USE_LEGACY_FACTORY
	vert_t* v1 = he->flip()->v;
...
void MeshRimFace::computePlaneCornerOnFace(
	vert_t* v, he_t* he, vector<QVector3D> &vmid, vector<QVector3D> &vpos)
{
#ifdef USE_LEGACY_FACTORY
	...
	for(vert_t &v: cur_mesh->verts())
	{
		...
		for (he_t* he: mesh->incidentEdges(&v))
		{
			...
			if (onEdge)
			{
				if (shapeType == 0)
				{
					...
					if (isQuadratic)
					{
						// construct vertex directly into verts_new
						// and assign reference ID to new vertex
						verts_new.emplace_back(vpos[2]);
						verts_new.back().refid = v.refid;
						verts_new.emplace_back(vpos[3]);
						verts_new.back().refid = v1->refid;
						verts_new.emplace_back(vpos[1]);
						verts_new.back().refid = v1->refid;
						verts_new.emplace_back(vpos[0]);
						verts_new.back().refid = v.refid;

						// record new vertices pointers
						auto vptr = &verts_new.back();
						vector<vert_t*> vertices = { vptr - 3,vptr - 2,vptr - 1, vptr };
			...
			else
			{
				if (shapeType == 0)
				{
					...
					{
						// construct vertex directly into verts_new
						verts_new.emplace_back(vpos[0]);//0
						verts_new.back().refid = v.refid;
						verts_new.emplace_back(vpos[1]);//1
						verts_new.back().refid = v1->refid;
						verts_new.emplace_back(vpos[3]);//3
						verts_new.back().refid = he->prev()->v->refid;
						verts_new.emplace_back(vpos[2]);//2
						verts_new.back().refid = v.refid;
				...
				else if(shapeType == 1)
				{
					...
					for (QVector3D pos: vpos)
					{
						...
						auto lastV_ptr = &verts_new.back();
						vertices.push_back(lastV_ptr);
						lastV_ptr->refid = ori_he->v->refid;
						ori_he = ori_he->next;
					}
		...
		if (isQuadratic || shapeType == 1)
		{
			...
			for(int i = 0; i < pieces.size(); i++)
			{
				...
				// get nextFace he2
				he_t* he2;
				if(shapeType == 0) he2 = nextFace->he->next()->next;
				else			   he2 = nextFace->he->prev;
	...
	for (face_t &f: cur_mesh->faces()) {
		...
		do {
			...
			for (int i = 0; i < 2; i++)
			{
				...
				// directly construct new vertices of new planar piece into verts_new
				// WARNING: refid not assigned
				if (i == 0)
				{
					verts_new.emplace_back(vpos_0);
					verts_new.emplace_back(vpos_1);
					verts_new.emplace_back(vpos_2);
					verts_new.emplace_back(vpos_3);
				}
				else
				{
					verts_new.emplace_back(vpos_1);
				...
				// Create vertices ptr array for face generation
				auto vptr = &verts_new.back();
				vector<vert_t*> vertices = { vptr - 3,vptr - 2,vptr - 1, vptr };
				// Create new face
				face_t* newFace = createFace(vertices, cutFace);
